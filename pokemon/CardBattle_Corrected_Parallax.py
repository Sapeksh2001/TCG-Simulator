...
def setup_parallax(canvas, root):
    ...
    def pygame_to_tk_image(pygame_surface):
        image_data = pygame.image.tostring(pygame_surface, 'RGB')
        image = Image.frombytes('RGB', pygame_surface.get_size(), image_data)
        return ImageTk.PhotoImage(image)

    # Update loop for parallax background
    def update_parallax():
        draw_bg()
        draw_ground()

        # Convert to Tkinter image and draw on canvas
        tk_image = pygame_to_tk_image(pygame_surface)
